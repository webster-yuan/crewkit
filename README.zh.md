...
> 打开 Claude Code 之前，用这个表判断。

| 你的任务 | 推荐组合 | 一句话原因 |
|---------|---------|-----------|
| 修 Bug、改配置、小调整（≤2 文件） | **只用 ECC** | 不需要流程纪律，别引入调度开销 |
| 日常小改动、代码巡检 | **只用 ECC** | 轻量，安全带就够 |
| 新功能、需求明确、改动适中 | **单会话** 或 **crewkit + Superpowers** | 上下文能装下就单会话，更快 |
| 新模块、架构重构、跨模块改动 | **crewkit + Superpowers** | 需要文档链 + 跨会话恢复 + 审计 |
| 需求模糊、需要先澄清再动手 | **crewkit + Superpowers** | BA 阶段逼你问清楚 |
| 多人协作、需防流氓代码 | **crewkit** | `/crewkit:review` 审计未文档化的改动 |

**不管你选哪个：ECC 的安全底线永远开着（但跟 crewkit 同时用时关闭 Instincts）。**

> 为什么？详见 [crewkit 在 Agent 生态系统中的定位](#crewkit-在-agent-生态系统中的定位)。
...
Claude Code 在上下文接近 200K token 时会自动压缩——把早期对话总结成一段话。摘要保留了「做了什么」，但丢了「为什么这么做」：微决策、被排除的方案、边界推理全没了。

crewkit 的文档链在压缩触发之前就**主动产出结构化记录**：

| | Claude Code 自动压缩 | crewkit 文档链 |
|---|---|---|
| 触发时机 | 被动触发（逼近上限） | 主动产出（每阶段结束时） |
| 保留什么 | 简短结论摘要 | 方案对比矩阵、优缺点、推荐理由、MUST 门禁结果 |
| 丢失什么 | 被排除的方案、微决策 | 强制要求保留（Architect 必须列 ≥2 个候选方案） |
| 可恢复性 | 丢失的推理无法回放 | 完整文档链——读 `docs/architect/` 就能看到每个决策 |

对于会触发自动压缩的 L 级任务，crewkit 的文档是「我们选了方案 B，因为排除了 A（>10K 用户时太慢）和 C（团队不熟悉）」和「我们选了方案 B」之间的差距。后一句话三天后可能导致错误决策。

---

## 已知局限

### 角色权重冲突：crewkit vs Claude Code 内置代理

crewkit 的角色定义在 **Skill 层（提示词层）**，Claude Code 的内置代理（如 `code-explorer`、`plan-agent`）在**基础设施层**。两层之间没有权重声明机制。

```
crewkit:  PM 派 Architect 做技术调研
             ↓ 冲突
Claude Code: 自动触发 code-explorer 搜索代码
```

**表现**：Architect 阶段可能被 code-explorer 绕过 PM 直接介入，两个角色做同一件事，甚至产出不一致。

**为什么修不了**：
- **软约束**（prompt 声明、CLAUDE.md 规则）→ 只是建议，无技术约束力
- **硬约束**（关闭自动子代理）→ 弱化了 Claude Code 的核心能力

这是一个 Skill 层的根本局限：crewkit 只能**建议** Claude 怎么做，不能**阻止** Claude Code 的运行时行为。

**当前建议**：PM 在 Worker 完成后做 gate-check 时，额外检查「是否有内置代理干扰」。若有，在 Worker 的 memory 中记录。

---

## crewkit 在 Agent 生态系统中的定位

### ECC vs crewkit：防止犯错 vs 确保做对

| | ECC | crewkit + Superpowers |
|---|---|---|
| **目标** | 防止 Agent **出错** | 确保 Agent **做对** |
| **手段** | 安全护栏（Hooks、Instincts、Security） | 流程纪律（PM 调度、Quality Gates、文档链） |
| **比喻** | 安全带——每次都要系 | 导航——去陌生地方才开 |
| **最佳场景** | 日常小改动、Bug 修复、代码巡检 | 新模块、架构重构、需求模糊、跨会话大任务 |
| **对 Agent 的态度** | 「别乱删文件」 | 「先想清楚要什么再动手」 |

**两者是互补的，不是替代关系。** ECC 保证 Agent 不搞破坏，crewkit 保证 Agent 产出高质量。

### 为什么 ECC + crewkit 直接叠加会冲突

ECC 和 crewkit 是**两个调度者**，没有互操作协议：

```
ECC 的 Instincts 拦截工具调用
  → crewkit Worker 无法应答 hooks
    → 流水线卡死

ECC 的自动 Skills 注入
  → crewkit Worker 的上下文被污染
    → 角色隔离设计被打破

ECC 的 Memory 与 crewkit 的 Memory
  → 两个系统互不知情
    → 经验碎片化
```

**结论**：ECC + crewkit 同时开启会互相削弱，不是增强。

### 最佳组合：crewkit + Superpowers（零冲突）

```
crewkit（调度层）   —— PM 决定谁做什么、用什么技能
Superpowers（技能层） —— 被动技能库，crewkit 说用哪个才用哪个
Claude Code（运行时） —— 执行，不做额外调度
```

**Superpowers 适配 crewkit 的原因**：
- ❌ 无 Hooks/Instincts → 不拦截 Worker 操作
- ❌ 无自动 Skills 注入 → 不污染 Worker 上下文
- ❌ 无独立 Memory → 不产生碎片
- ✅ 纯技能集合 → crewkit 的 Dynamic Skill Discovery 直接从 Superpowers 挑技能

### 实际使用建议

| 场景 | 推荐组合 | 原因 |
|------|---------|------|
| S 级（Bug fix、配置调整） | **只用 ECC** | 不需要流程纪律，安全带就够 |
| 日常小改动 | **只用 ECC** | 轻量，不引入调度开销 |
| M 级（新功能、需求明确） | **crewkit + Superpowers** 或 **单会话** | 视上下文能否装下，优先单会话 |
| L 级（新模块、架构重构） | **crewkit + Superpowers** | 需要文档链、跨会话、审计 |
| 任何场景 | **ECC 始终在线**（安全底线） | 但不能跟 crewkit 同时开启 Instincts |

### 一句话总结

> **ECC 是安全带，每次开车都要系。crewkit 是导航，去陌生城市才开。Superpowers 是地图数据，导航要用的时候随时取。**
